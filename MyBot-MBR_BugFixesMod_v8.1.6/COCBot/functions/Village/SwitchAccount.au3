...
Func SwitchCOCAcc_DisconnectConnect(ByRef $bResult, $bDisconnectOnly = $g_bChkSharedPrefs)
	...
	For $i = 0 To 20 ; Checking Green Connect Button continuously in 20sec
		...
		If IsArray($avGoogleButtonStatus) And UBound($avGoogleButtonStatus, 1) > 0 Then
			...
			EndIf
		Else ; SupercellID
			Local $aSuperCellIDConnected = decodeSingleCoord(findImage("SupercellID Connected", $g_sImgSupercellIDConnected, GetDiamondFromRect("630,150,780,200"), 1, True, Default))
...
Func SwitchCOCAcc_ClickAccountSCID(ByRef $bResult, $NextAccount, $iStep = 2, $bVerifyAcc = True, $bDebuglog = $g_bDebugSetlog, $bDebugImageSave = $g_bDebugImageSave)
	Local $sAccountDiamond = GetDiamondFromRect("545,385,575,725") ; Contains iXStart, $iYStart, $iXEnd, $iYEnd
	...
	For $i = 0 To 30 ; Checking "New SuperCellID UI" continuously in 30sec
		$aSuperCellIDWindowsUI = decodeSingleCoord(findImage("SupercellID Windows", $g_sImgSupercellIDWindows, GetDiamondFromRect("715,90,775,150"), 1, True, Default))
		If _Sleep(500) Then Return "Exit"
...
Func SCIDragIfNeeded($iSCIDAccount, $bVerifyAcc = True)
	...
	ClickDrag($x1, $y, $x2, $y - (100 * ($iSCIDAccount - 3)), 500, True) ; drag a multiple of 90 pixels up for how many accounts down it is
